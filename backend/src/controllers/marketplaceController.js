...
// Includes pickup_lat, pickup_lng, pickup_address from users table
// so buyers can see the homecook location on map
// ==========================================
  ...
  try {
    ...
    let query = `
      SELECT
        ...
        u.pickup_lat   AS pickup_lat,
        u.pickup_lng   AS pickup_lng,
        u.pickup_address AS pickup_address
    ...
    if (search) {
      query += ` AND (
        LOWER(r.recipe_name) LIKE LOWER($${p}) OR
        LOWER(r.description)  LIKE LOWER($${p}) OR
        LOWER(u.full_name)    LIKE LOWER($${p})
      )`;
    ...
    res.json({ success: true, data: { listings: result.rows, count: result.rows.length } });
  } catch (error) {
    console.error('❌ Error getting marketplace listings:', error);
    // If pickup_lat/lng columns don't exist yet, fall back without them
    if (error.code === '42703') {
      try {
        ...
        return res.json({ success: true, data: { listings: fallback.rows, count: fallback.rows.length } });
      } catch (e2) {
...
// ==========================================
// PLACE ORDER — dynamically detects your orders table columns
// so it never crashes on missing columns
// ==========================================
exports.placeOrder = async (req, res) => {
  try {
    const buyerId = req.user.id;
    const { recipe_id, quantity, pickup_date, pickup_time, special_notes } = req.body;
    console.log('📥 Place order request:', { buyerId, recipe_id, quantity, pickup_date });
    ...
    // Detect actual columns in orders table to avoid column-not-found errors
    ...
    const tryAdd = (col, val) => {
      if (cols.includes(col) && val !== undefined && val !== null && val !== '') {
        insertCols.push(col); insertVals.push(val);
      }
    };

    tryAdd('pickup_date',      pickup_date);
    tryAdd('pickup_time',      pickup_time);
    tryAdd('pickup_datetime',  pickup_date ? `${pickup_date}${pickup_time ? ' ' + pickup_time : ''}` : null);
    tryAdd('special_notes',    special_notes);
    tryAdd('special_requests', special_notes);
    tryAdd('notes',            special_notes);
...
exports.submitReview = async (req, res) => {
  try {
    ...
    const orderCheck = await pool.query('SELECT * FROM orders WHERE id=$1 AND status=$2', [order_id, 'completed']);
    ...
    const cols = colResult.rows.map(r => r.column_name);
    const commentCol = cols.includes('comment') ? 'comment' : cols.includes('review_text') ? 'review_text' : 'comment';
